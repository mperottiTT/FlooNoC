...
module floo_meta_buffer #(
  ...
  if (MaxUniqueIds == 1) begin : gen_no_atop_fifos
    ...
    `TT_OPENSOURCE_ASSERT(NoBResponseIdQueue, axi_rsp_i.b_valid -> (b_oup_data_valid && b_oup_gnt))
    `TT_OPENSOURCE_ASSERT(NoRResponseIdQueue, axi_rsp_i.r_valid -> (r_oup_data_valid && r_oup_gnt))
  end
  ...
  `TT_OPENSOURCE_ASSERT(NoAtopSupportAw, !(!AtopSupport && is_atop_aw))
